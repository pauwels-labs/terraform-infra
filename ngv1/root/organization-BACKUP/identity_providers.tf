# resource "keycloak_oidc_identity_provider" "users" {
...
#   client_id         = "realm-${keycloak_realm.environments[each.key].realm}"
#   client_secret     = keycloak_openid_client.environments[each.key].client_secret
