provider "kubernetes" {
  alias                  = "c4"
  host                   = local.cluster_create[local.cluster_index[4]] ? module.c4[0].cluster_endpoint : ""
  cluster_ca_certificate = base64decode(local.cluster_create[local.cluster_index[4]] ? module.c4[0].cluster_certificate_authority_data : "")
  exec {
...
module "c4" {
  ...
  cluster_name                    = local.cluster_names[local.cluster_index[4]]
  cluster_version                 = var.cluster_version
  cluster_endpoint_private_access = true
  cluster_endpoint_public_access  = true
  cluster_tags                    = {
    Name        = local.cluster_names[local.cluster_index[4]]
  ...
  cluster_encryption_config                 = [
    {
      provider_key_arn = aws_kms_key.envelope_encryption[local.cluster_index[4]].arn
      resources        = [
        "secrets"
      ]
    }
  ...
  eks_managed_node_groups = {
    for i, subnet in slice(aws_subnet.private, local.cluster_index[4] * local.az_count, local.cluster_index[4] * local.az_count + local.az_count) : subnet.availability_zone => {
      name        = "${local.cluster_names[local.cluster_index[4]]}-${subnet.availability_zone}"
      description = "EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[4]]} cluster scoped to the ${subnet.availability_zone} AZ"
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[4]]}-${aws_subnet.private[0].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[4]]} cluster scoped to the ${aws_subnet.private[0].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[4]]}-${aws_subnet.private[1].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[4]]} cluster scoped to the ${aws_subnet.private[1].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[4]]}-${aws_subnet.private[2].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[4]]} cluster scoped to the ${aws_subnet.private[2].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[4]]}-${aws_subnet.private[3].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[4]]} cluster scoped to the ${aws_subnet.private[3].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[4]]}-${aws_subnet.private[4].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[4]]} cluster scoped to the ${aws_subnet.private[4].availability_zone} AZ", "")
