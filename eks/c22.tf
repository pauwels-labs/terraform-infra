provider "kubernetes" {
  ...
  exec {
    ...
    command     = "aws"
    args        = ["eks", "get-token", "--cluster-name", local.cluster_create[local.cluster_index[22]] ? module.c22[0].cluster_name : "", "--region", var.cluster_region, "--profile", "mfa", "--role-arn", local.cluster_account_role_arn]
  }
}
...
module "c22" {
  ...
  cluster_name                    = local.cluster_names[local.cluster_index[22]]
  cluster_version                 = var.cluster_version
  cluster_endpoint_private_access = true
  cluster_endpoint_public_access  = true
  cluster_tags                    = {
    Name        = local.cluster_names[local.cluster_index[22]]
  ...
  eks_managed_node_groups = {
    for i, subnet in slice(aws_subnet.private, local.cluster_index[22] * local.az_count, local.cluster_index[22] * local.az_count + local.az_count) : subnet.availability_zone => {
      name        = "${local.cluster_names[local.cluster_index[22]]}-${subnet.availability_zone}"
      description = "EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[22]]} cluster scoped to the ${subnet.availability_zone} AZ"
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[22]]}-${aws_subnet.private[0].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[22]]} cluster scoped to the ${aws_subnet.private[0].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[22]]}-${aws_subnet.private[1].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[22]]} cluster scoped to the ${aws_subnet.private[1].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[22]]}-${aws_subnet.private[2].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[22]]} cluster scoped to the ${aws_subnet.private[2].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[22]]}-${aws_subnet.private[3].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[22]]} cluster scoped to the ${aws_subnet.private[3].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[22]]}-${aws_subnet.private[4].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[22]]} cluster scoped to the ${aws_subnet.private[4].availability_zone} AZ", "")
