provider "kubernetes" {
  ...
  exec {
    ...
    command     = "aws"
    args        = ["eks", "get-token", "--cluster-name", local.cluster_create[local.cluster_index[24]] ? module.c24[0].cluster_name : "", "--region", var.cluster_region, "--profile", "mfa", "--role-arn", local.cluster_account_role_arn]
  }
}
...
module "c24" {
  ...
  cluster_name                    = local.cluster_names[local.cluster_index[24]]
  cluster_version                 = var.cluster_version
  cluster_endpoint_private_access = true
  cluster_endpoint_public_access  = true
  cluster_tags                    = {
    Name        = local.cluster_names[local.cluster_index[24]]
