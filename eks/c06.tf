provider "kubernetes" {
  ...
  #host = module.c6[0].cluster_endpoint
  ...
  #cluster_ca_certificate = base64decode(module.c6[0].cluster_certificate_authority_data)
  exec {
...
module "c6" {
  ...
  cluster_name                    = "${var.cluster_name}-${local.cluster_index[6]}"
  cluster_version                 = var.cluster_version
  cluster_endpoint_private_access = true
  cluster_endpoint_public_access  = true
  cluster_tags                    = {
    Name        = "${var.cluster_name}-${local.cluster_index[6]}"
  ...
  eks_managed_node_groups = {
    for subnet in slice(aws_subnet.private, local.cluster_index[6] * local.az_count, local.cluster_index[6] * local.az_count + local.az_count) : subnet.availability_zone => {
      name        = "${var.cluster_name}-${local.cluster_index[6]}-${subnet.availability_zone}"
      description = "EKS node group in a private subnet for the ${var.cluster_name}-${local.cluster_index[6]} cluster scoped to the ${subnet.availability_zone} AZ"
