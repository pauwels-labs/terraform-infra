provider "kubernetes" {
  ...
  exec {
    ...
    command     = "aws"
    args        = ["eks", "get-token", "--cluster-name", local.cluster_create[local.cluster_index[1]] ? module.c1[0].cluster_name : "", "--region", var.cluster_region, "--profile", "mfa", "--role-arn", local.cluster_account_role_arn]
  }
}
...
module "c1" {
  ...
  iam_role_name            = local.cluster_iam_role_name[local.cluster_index[1]]
  ...
  cluster_encryption_policy_name            = local.cluster_encryption_policy_name[local.cluster_index[1]]
  cluster_encryption_policy_use_name_prefix = false
  cluster_encryption_config                 = [
    {
      provider_key_arn = aws_kms_key.envelope_encryption[local.cluster_index[1]].arn
      resources        = [
        "secrets"
      ]
    }
  ...
  eks_managed_node_groups = {
    for i, subnet in slice(aws_subnet.private, local.cluster_index[1] * local.az_count, local.cluster_index[1] * local.az_count + local.az_count) : subnet.availability_zone => {
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[1]]}-${aws_subnet.private[0].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[1]]} cluster scoped to the ${aws_subnet.private[0].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[1]]}-${aws_subnet.private[1].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[1]]} cluster scoped to the ${aws_subnet.private[1].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[1]]}-${aws_subnet.private[2].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[1]]} cluster scoped to the ${aws_subnet.private[2].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[1]]}-${aws_subnet.private[3].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[1]]} cluster scoped to the ${aws_subnet.private[3].availability_zone} AZ", "")
    ...
    #   name        = try("${local.cluster_names[local.cluster_index[1]]}-${aws_subnet.private[4].availability_zone}", "")
    #   description = try("EKS node group in a private subnet for the ${local.cluster_names[local.cluster_index[1]]} cluster scoped to the ${aws_subnet.private[4].availability_zone} AZ", "")
