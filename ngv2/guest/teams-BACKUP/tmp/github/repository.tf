...
resource "github_repository_webhook" "cd" {
  ...
  configuration {
    url          = "https://webhook.cd.${each.value.name}.${var.webhook_domain}/hook/${sha256("${random_password.hmac_tokens["primary"].result}${var.team_repository_domain}-${var.organisation_name}-team-${var.team_name}${var.team_name}")}"
    content_type = "json"
