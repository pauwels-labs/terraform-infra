resource "vault_kv_secret_v2" "root" {
  mount               = "secrets"
  name                = "tenants/t-${local.full_tenant_name}/root"
...
resource "vault_kv_secret_v2" "hmac_tokens" {
  ...
  mount               = "secrets"
  name                = "tenants/t-${local.full_tenant_name}/webhooks/${each.key}/hmac"
  delete_all_versions = true
  data_json           = jsonencode({
...
resource "vault_kv_secret_v2" "repository_bot_token" {
  mount               = "secrets"
  name                = "tenants/t-${local.full_tenant_name}/tokens/${var.repository_bot_host}/${var.repository_bot_name}"
  delete_all_versions = true
  data_json           = jsonencode({
    "token" = var.repository_bot_token
  })
}

resource "vault_kv_secret_v2" "repository_ssh_cd" {
  mount               = "secrets"
  name                = "tenants/t-${local.full_tenant_name}/repositories/${var.repository_bot_host}/${var.org_name}/${github_repository.this.name}/ssh/cd"
  delete_all_versions = true
  data_json           = jsonencode(
    {
...
resource "vault_kv_secret_v2" "repository_ssh_ci" {
  mount               = "secrets"
  name                = "tenants/t-${local.full_tenant_name}/repositories/${var.repository_bot_host}/${var.org_name}/${github_repository.this.name}/ssh/ci"
  delete_all_versions = true
  data_json           = jsonencode(
    {
